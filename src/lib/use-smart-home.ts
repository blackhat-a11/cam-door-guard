...
const truthy = (v: string) => ["1", "on", "true", "open", "ON", "HIGH"].includes(v.trim());
...
export function useSmartHome() {
  ...
  const handleMessage = useCallback(
    (topic: string, payloadBuf: Uint8Array) => {
      ...
      setTraffic((prev) => [{ topic, payload: payload.slice(0, 300), at }, ...prev].slice(0, 60));
      ...
      switch (key) {
        ...
        case "sensor/ir":
        case "ir/state":
          setState((s) => ({ ...s, ir: truthy(payload) }));
          return;
        case "door/state":
  ...
  const publish = useCallback(
    (subTopic: string, payload: string) => {
      ...
      if (!client || conn !== "online") return false;
      client.publish(topic, payload, { qos: 0 });
      setTraffic((prev) =>
        [{ topic: `→ ${topic}`, payload, at: new Date().toISOString() }, ...prev].slice(0, 60),
      );
      return true;
    },
    [settings.base, conn],
